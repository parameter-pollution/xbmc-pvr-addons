...
std::string A1TVData::GetSettingsFile() const
{
  string settingFile = g_strClientPath;
  if (settingFile.at(settingFile.size() - 1) == '\\' ||
      settingFile.at(settingFile.size() - 1) == '/')
    settingFile.append("A1TVAddonSettings.xml");
  else
    settingFile.append("/A1TVAddonSettings.xml");
  return settingFile;
}

bool A1TVData::LoadDemoData(void)
{
  ...
  if (pElement)
  {
    ...
    while ((pEpgNode = pElement->IterateChildren(pEpgNode)) != NULL)
    {
      ...
      if (!XMLUtils::GetInt(pEpgNode, "channelid", iTmp))
        continue;
      A1TVChannel &channel = m_channels.at(iTmp - 1);
...
PVR_ERROR A1TVData::GetEPGForChannel(ADDON_HANDLE handle, const PVR_CHANNEL &channel, time_t iStart, time_t iEnd)
{
  ...
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    ...
    while (iLastEndTime < iEnd && myChannel.epg.size() > 0)
    {
      ...
      {
        A1TVEpgEntry &myTag = myChannel.epg.at(iEntryPtr);
...
int A1TVData::GetRecordingsAmount(void)
{
  return m_recordings.size();
}

PVR_ERROR A1TVData::GetRecordings(ADDON_HANDLE handle)
{
  for (std::vector<A1TVRecording>::iterator it = m_recordings.begin() ; it != m_recordings.end() ; it++)
  {
    A1TVRecording &recording = *it;
